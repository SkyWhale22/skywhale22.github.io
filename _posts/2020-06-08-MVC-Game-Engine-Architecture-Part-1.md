...
각각의 항목들은 API에 따라 또는 플랫폼에 따라 확연한 차이가 있을 수 있다. 사용하는 플랫폼에 따라 분리된 코드들을 알아서 연결해주고, 새로운 플랫폼에 대한 지원을 추가할 때, 엔진의 모든 구조를 바꿀 필요 없이 해당 플랫폼의 코드만 추가해주면 되는 레이어를 두면, 다음에 발생할 수 있는 작업을 최소화 할 수 있다.
...
# **코드를 봅시다** 
한동안 포스팅을 멈춘 동안, 주변 개발자들과 앞으로의 블로그 포스팅의 방향에 관하여 이야기를 나누었다. 어차피 Github에 엔진의 전체 코드를 올릴 테니, 굳이 여기에서 코드를 쭉 나열하고 설명하는 것보단, 클래스 정의만 활용하여 어떤 구조로 되어있는지와 어떤 메소드들이 필요한지 설명하고, 독자들이 자신만의 엔진을 만들어 보는 계기를 주는 게 좀 더 이 시리즈의 의의와 걸맞다고 판단하였다. 그렇기에 각 클래스의 구현체는 해당 블로그에서 최대한 다루지 않을 예정이다. 구현 부분을 보고 싶다면, 추후 내 Github를 확인해보길 바란다.
...
<span style="color:green">**ISystem**</span>안에 있는 대부분의 순수 가상 함수들은 사용자의 하드웨어 사양을 확인하는 함수들과 파일 디렉토리 관련 함수들이다. 이들은 특정 플랫폼에 한정된 기능 호출을 요구로 한다. 이 부분들의 구현은 직접 해보도록 하자.
<br><br><br>
...
Foo가 24바이트, Bar가 16바이트가 나오는 이유는 메모리 정렬 규칙에 의해 내부에 패딩이 들어가기 때문이다.
...
struct Foo
{
    ...
    bool  m_bool;           // 1 Bytes -+
    char  padding_1[3];     // 3 Bytes  |-- 8 Bytes
    int   m_int;            // 4 Bytes -+

    bool  m_anotherBool;    // 1 Bytes -+-- 8 Bytes
    char  padding_2[7];     // 7 Bytes -+
};                          // 총 24 bytes
...
+---+---+---+---+---+---+---+---+---+---+---+---+---+---+---+---+---+---+---+---+---+---+---+---+
| 0 | 0 | 0 | 0 | 0 | 0 | 0 | 0 | 0 | c | c | c | 0 | 0 | 0 | 0 | 0 | c | c | c | c | c | c | c | 
+---+---+---+---+---+---+---+---+---+---+---+---+---+---+---+---+---+---+---+---+---+---+---+---+

|_______________________________|___|___________|_______________|___|___________________________|
            char*               bool   padding         int       bool        padding
|_______________________________|_______________________________|_______________________________|
          8 Bytes                           8 Bytes                          8 Bytes             

메모리 정렬 규칙.
1. x86는 32비트 (== 4 bytes), x64는 64비트(== 8 bytes)의 바운더리를 가진다.
2. 멤버 변수들은, 그 사이즈의 배수에 위치하여야 한다.
3. 오브젝트의 전체 사이즈는, 가장 큰 사이즈의 배수가 되어야 한다.

(심화) False sharing을 피하기 위해 2^n의 사이즈를 가지고 있어야한다.
...
- int형의 사이즈는 4 바이트이다. 이에 4의 배수인 12부터 시작해야한다.
  그렇기에 bool과 int 사이에 3바이트의 패딩을 집어넣는다.
...
- 메모리 정렬 규칙 3번에 의해, 오브젝트의 전체 사이즈는, 가장 큰 사이즈의 배수가 되어야한다.
  구조체 Foo의 가장 큰 멤버 변수의 사이즈는 8 bytes이다. 이에 Foo의 최종 사이즈는 8의 배수가 되어야 한다.
  그렇기에 8의 배수인 24가 되기 위해 7바이트의 패딩을 집어넣는다.
...
- 9 % 4 = 1. 나누어 떨어지지 않는다. 이에 이는 잘못된 패딩이다.
...
+---+---+---+---+---+---+---+---+---+---+---+---+---+---+---+---+
|_______________________________|_______________|___|___|_______|
            char*                     int       bool bool  padding
...
False Sharing
- 구조체의 크기가 2의 제곱수로 맞춰지지 않은 경우, 가속화시 문제가 발생할 수 있다.
- CPU의 코어들은 각자 자신만의 L1 Cache를 가지고 있으며, 일반적으로 64 bytes의 CacheLine 이라는 Cache의 가장 작은 단위를 가지고 있다.
  이 구조 때문에, 데이터를 64 bytes 단위로 쪼개게 된다.

- 구조체 Foo처럼 총 사이즈가 24 바이트인 경우, 메모리에 정렬된 한 덩어리가 둘로 쪼개지게된다.
  +-------+-------+-------+-------+-------+-------+
  |  Foo  |  Foo  |  Foo  |  Foo  |  Foo  |  Foo  |
  +-------+-------+-------+-------+-------+-------+
  0       24      48      72      96      120     144
  |___________________|____________________|
   CacheLine (64 bytes) CacheLine (64 bytes)
                      ^
                      |_____ False Sharing 발생.


이 문제를 해결하기 위해, C++11 부터 alignas()라는 지정자가 스탠다드에 추가 되었다. 2의 제곱수에 맞게 지정하면, 추가로 패딩을 넣어준다.
*/
struct alignas(32) Foo
{
    ...
    // alignas() 지정자로 인해 새로 추가되는 패딩
    char  padding_3[8];     // 8 Bytes
};                          // 총 32 bytes
``` -->